...
import os
import torch 
from retrieval.data_util.core.data_dealer import TextDataDealer
from retrieval.data_util.core.gen_data import load_ameli_test_data_for_cross_encoder
from retrieval.data_util.core.help_util import load_qrels
...
def load_top_1000_corpus( top_candidate_corpus_path,queries,corpus):
    ...
    for query_relative_position, score_dict_list in enumerate(queries_result_list ):
        top_hits = sorted(queries_result_list[query_relative_position], key=lambda x: x['score'], reverse=True)
        top_hits=top_hits[:1000] 
        qid=query_id_list[query_relative_position]
...
def test_cross_encoder(args,match_path=None,feature_extractor_path=None):
    if match_path is None:
        model_name=args.model_name
    else:
        model_name=match_path
    ...
    queries,relevant_docs,relevant_qid,corpus=load_ameli_test_data_for_cross_encoder(args.test_data_folder,args)
    ...
    model.load( model_name, feature_extractor_path)
    model.model.cuda()
    ...
    for qid in tqdm.tqdm(relevant_qid):
        ...
        for c in cand:
            if   args.media=="img":
                corpus_sentences.append(Image.open(c[1]))
        ...
        if model.config.num_labels > 1: #Cross-Encoder that predict more than 1 score, we use the last and apply softmax
            ...
            cross_scores = model.predict(cross_inp,batch_size=512 ).tolist()
