...
class SemanticSearcher:
    ...
    def encode_corpus(self,document_sent_list):
        ...
        self.corpus_embeddings = self.bi_encoder.encode(document_sent_list,batch_size=200, convert_to_tensor=True, show_progress_bar=True)
    ...
    def search(self,query,top_k):
        ...
        if self.no_rerank:
            ...
            try:
                cross_scores = self.cross_encoder.predict(cross_inp,batch_size=1024,show_progress_bar=False)
            except Exception as e:
                print(f"cross_encoder {e}")
                raise e
            # Sort results by the cross-encoder scores
            for idx in range(len(cross_scores)):
                hits[idx]['cross-score'] = cross_scores[idx]
                hits[idx]['cross_score_sigmoid'] =cross_score_to_sigmoid(cross_scores[idx])
