    ...
    if idx>=start_idx or args.is_slice_start_to_end=="y" :
        if   idx<end_idx or args.is_slice_start_to_end=="y":
            ...
            if f"predicted_attribute_{args.attribute_logic}" not in review_dataset_json or is_allow_override:
                product_id=review_dataset_json["asin"]
                review_json=review_dataset_json
                review_id=review_json["reviewerID"]
                # mention=review_json["mention"]
                mention=""
                if "reviewText" not in review_json:
                    should_skip=True
                    return  should_skip,output_list ,acc_dict,predict_num_dict
                review=review_json["reviewText"] 
                if "summary" in review_json:
                    review=review_json["summary"]+". "+review
                if product_id not in product_dict:
                    if product_id not in valid_product_id_list:
                        print(product_id)
                    should_skip=True
                    return  should_skip,output_list ,acc_dict,predict_num_dict
                gold_product_json=product_dict[product_id]
                all_ocr_raw,all_gpt_context,all_chatgpt_context={},{},{}
                gold_attribute_json=gen_gold_attribute_without_key_section(gold_product_json["tech1"],gold_product_json["tech2"]  )
...
def gen_amazon_product_dict(product_path):
    valid_product_id_list=[]
    product_dict={}
    with gzip.open(product_path, "rt") as input_file :
...
def main(args,data_path,output_products_path, products_path_str,review_id_to_debug):
    ...
    product_dict,valid_product_id_list=gen_amazon_product_dict( products_path_str)
    ...
    if args.data_format=="gz":
        generate_for_gz_dataset(data_path,args, 
...
def bs4_review_scraper(review_dataset_json,extractor,args, product_dict,test_val_gpt_attribute_set):
    ...
    gold_product_json=product_dict[product_id]
    attribute_json=gen_gold_attribute_without_key_section(gold_product_json["tech1"],gold_product_json["tech2"] )
    if args.attribute_source=="similar":
...
def mp_main(args):
    ...
    product_dict,_=gen_amazon_product_dict( args.product_path)
...
def parser_args():
    ...
    parser.add_argument('--data_path',type=str,help=" ",default="data/amazon/allow_no_attribute/sampled_10000_Amazon_Review_allow_no_attribute.json")
    parser.add_argument('--out_path',type=str,help=" ",default="data/amazon/allow_no_attribute/sampled_10000_Amazon_Review_allow_no_attribute_w_attribute_vicuna")
    parser.add_argument('--attribute_logic',type=str,help=" ",default="vicuna_complete")#exact numeral amazon_exact
    parser.add_argument('--product_path',type=str,help=" ", default="data/amazon/All_Amazon_Product_w_Attribute.json.gz")
