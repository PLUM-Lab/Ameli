...
from attribute.extractor.gen_review_attribute_by_rule import MatchExtractor
...
from disambiguation.data_util.inner_util import gen_gold_attribute_without_key_section, json_to_dict, review_json_to_product_dict, spec_to_json_attribute, spec_to_json_attribute_w_special_attribute_success_flag
...
def match_for_multiple_extraction( attribute_value_candidate_list,attribute_value_gold_list,acc_dict,predict_num_dict,attribute_key):
    for attribute_value_candidate in attribute_value_candidate_list:
        ...
        #             return True 
    return acc_dict,predict_num_dict 
...
def generate_for_dataset(data_path,args,  output_products_path,
                          product_dict,is_constrained_beam_search,extractor,review_id_to_debug,is_allow_override=False):
    new_crawled_data_json_path = Path(
        data_path
    )
    ...
    with open(new_crawled_data_json_path, 'r', encoding='utf-8') as fp:
        ...
        for idx,review_dataset_json in enumerate(tqdm(review_dataset_json_array_to_use)):
            ...
            if idx>=start_idx or args.is_slice_start_to_end=="y" :
                if   idx<end_idx or args.is_slice_start_to_end=="y":
                    ...
                    if f"predicted_attribute_{args.attribute_logic}" not in review_dataset_json or is_allow_override:
                        ...
                        review=review_json["header"]+". "+review_json["body"]
                        gold_product_json=product_dict[product_id]
                        all_ocr_raw,all_gpt_context,all_chatgpt_context=list(review_json["confidence_score_ocr"].keys()),{},{}
                        ...
                        for attribute_key in  attribute_key_list :
                            ...
                            if args.attribute_field !="all" and attribute_key not in args.attribute_field:
                                continue
                            # else:
                            #     print(f"{args.attribute_field}")
                            # if attribute_key not in test_val_gpt_attribute_set and  args.attribute_logic=="chatgpt":
                            #     if not is_first_non_selected_attribute:
                            #         continue
                            #     else:
                            #         is_first_non_selected_attribute=False
                            ...
                            if attribute_key=="Model Version" :
                                acc_dict,predict_num_dict=match_for_multiple_extraction( attribute_value_candidate_list,gold_attribute_value,acc_dict,predict_num_dict,attribute_key)
                            else:    
                            ...
                            # if match( attribute_value_candidate_list_out_of_top10,gold_attribute_value):
                            #     # acc_dict[attribute_key]+=1
                            #     is_attribute_correct_out_of_top10="y"
                            #     # print(f"Right: predicted:{generated_sequence}, gold:{attribute_value}, predicted_by_stringmatch:{attribute_value_predicted_by_stringmatch}")
                            # else:
                            #     # is_match_all_attribute=False 
                            #     is_attribute_correct_out_of_top10="n"
...
def main(args,data_path,output_products_path, product_path,review_id_to_debug):
    ...
    product_dict=init2( products_path_str,args)
    ...
    generate_for_dataset(data_path,args, 
                          ...
                          product_dict,is_constrained_beam_search,extractor,review_id_to_debug ,is_allow_override=True)
...
def bs4_review_scraper(review_dataset_json,extractor,args, product_dict,test_val_gpt_attribute_set):
    ...
    review=review_json["header"]+". "+review_json["body"]
    gold_product_json=product_dict[product_id]
    attribute_json=gen_gold_attribute_without_key_section(gold_product_json["Spec"],gold_product_json["product_name"] ,{})
    if args.attribute_source=="similar":
...
def parser_args():
    ...
    parser.add_argument('--data_path',type=str,help=" ",default="/home/menglong/workspace/code/multimodal/multimodal_entity_linking/product_scraper/dataset_construction/bestbuy/data/final/v6/val/disambiguation/bestbuy_review_2.3.17.11.19.16.1_update_candidate.json")
    parser.add_argument('--out_path',type=str,help=" ",default="/home/menglong/workspace/code/multimodal/multimodal_entity_linking/product_scraper/dataset_construction/bestbuy/data/final/v6/test/temp/temp_bestbuy_review_2.3.17.11.19.1_similar_ocr")
    parser.add_argument('--attribute_logic',type=str,help=" ",default="vicuna")#exact numeral gpt2
    
    # parser.add_argument('--example_type',type=str,help=" ",default="verification")
    # parser.add_argument('--mode',type=str,help=" ",default="test")
    parser.add_argument('--attribute_source',type=str,help=" ",default="gold_attribute_category_similar_attribute_value")#gold
    ...
    parser.add_argument("--is_slice_start_to_end", type=str, default="y", help="Text added prior to input.")
    ...
    parser.add_argument("--prefix", type=str, default="Attribute Value Extraction:\n ", help="Text added prior to input.")
...
if __name__ == '__main__':
    ...
    if args.is_mp=="y":
        ...
        main(args,args.data_path, args.out_path, products_path_str,args.review_id_to_debug)