...
import argparse
import torch
from disambiguation.model_upload import ELModel2
...
from retrieval.data_util.core.gen_data import ImageDataDealer
# from retrieval.data_util.core.dataset import  MSMARCODataset, MSMARCOImageDataset, MultimodalDataset
from retrieval.data_util.core.data_dealer import  WikidiverseDataDealer, TextDataDealer 
from retrieval.data_util.core.help_util import  get_train_queries, load_qrels
from retrieval.eval.eval_msmarco_mocheg import gen_evaluator,     test_retrieval
 
 

def load_model(args,model_name,max_seq_length,device):
    # Load our embedding model
     
    logging.info("Create new SBERT model")
    model=ELModel2( args, device)
...
def load_data( args,ce_score_margin,num_negs_per_system):
    ...
    train_dataset = WikidiverseDataDealer(train_queries ,corpus)
...
def train(args):
    ...
    model_save_path,args=setup_with_args(args,'retrieval/output/runs_3','train_bi-encoder-{}'.format( datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
